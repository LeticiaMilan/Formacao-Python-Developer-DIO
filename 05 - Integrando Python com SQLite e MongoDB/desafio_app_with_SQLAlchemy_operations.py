...
def insert_users_data(session):
    leticia = Usuario(
        nome='leticia',
        nome_completo='Leticia Milan',
        endereco=[Endereco(endereco_email='leticia@example.com')]
    )

    joao = Usuario(
        nome='joao',
        nome_completo='Joao Vitor Mine',
        endereco=[Endereco(endereco_email='joaovitor@example.com'),
                Endereco(endereco_email='joaovitor@example.com')]
    )

    larry = Usuario(nome='larry', nome_completo='Larry Butterfly')

    session.add_all([leticia, joao, larry])
    session.commit()


def retrieve_users_by_name(session):
    stmt = session.query(Usuario).filter(Usuario.nome.in_(["leticia", 'joao']))
